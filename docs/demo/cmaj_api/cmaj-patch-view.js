...
export function getAvailableViewTypes(patchConnection) {
  ...
  if (patchConnection.manifest?.view?.src) return ["custom", "generic"];
...
export async function createPatchView(patchConnection, preferredType) {
  if (patchConnection?.manifest) {
    ...
    if (view && preferredType === "generic") if (view.src) view = undefined;

    const viewModuleURL = view?.src
      ? patchConnection.getResourceAddress(view.src)
    ...
    if (patchView) {
      ...
      if (view?.width > 10) patchView.style.width = view.width + "px";
      else patchView.style.width = undefined;

      if (view?.height > 10) patchView.style.height = view.height + "px";
      else patchView.style.height = undefined;
...
export async function createPatchViewHolder(patchConnection, preferredType) {
  ...
  if (view) {
    ...
    if (!window.customElements.get(patchViewHolderName))
      window.customElements.define(patchViewHolderName, PatchViewHolder);
